...
import { injectSocket } from './websocket.js';
...
const server = injectSocket(http.createServer())

const webServer = polka({ server }).use(handler);

webServer.listen({ path, host, port }, () => {
	console.log(`Listening on ${path ? path : host + ':' + port}`);
});

export { host, path, port, webServer };
